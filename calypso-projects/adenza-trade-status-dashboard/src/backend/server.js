...
function broadcastTrades(filters = {}) {
  AdenzaClient.getTrades(filters)
    .then(trades => {
...
app.get('/api/trades', async (req, res) => {
  try {
    ...
    const trades = await AdenzaClient.getTrades(filters);
    res.json(trades);
  } catch (error) {
...
app.get('/api/trades/:id', async (req, res) => {
  try {
    const trade = await AdenzaClient.getTradeById(req.params.id);
    if (trade) {
      res.json(trade);
    } else {
...
// Initialize Adenza client
AdenzaClient.authenticate()
  .then(() => {
  ...
  .catch(error => {
    logger.error('Failed to initialize Adenza client', error);
    process.exit(1);
  });
