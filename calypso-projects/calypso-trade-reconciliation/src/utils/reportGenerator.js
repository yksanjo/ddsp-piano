    ...
    discrepancies.forEach((disc, index) => {
      ...
      if (disc.type === 'field_mismatch') {
        disc.fields.forEach(field => {
          ...
          report += `     Calypso: ${field.calypso}\n`;
          report += `     External: ${field.external}\n`;
        });
      } else if (disc.type === 'missing_in_external') {
        report += `   Trade exists in Calypso but not in external file\n`;
        report += `   Calypso Data: ${JSON.stringify(disc.calypso, null, 2)}\n`;
      } else if (disc.type === 'missing_in_calypso') {
        report += `   Trade exists in external file but not in Calypso\n`;
