...
class CalypsoClient {
  constructor() {
    ...
    this.authType = config.calypso.authType;
  ...
  async authenticate() {
    ...
    if (this.authType === 'oauth2') {
      try {
        const response = await axios.post(config.calypso.oauth.tokenUrl, {
          client_id: config.calypso.oauth.clientId,
          client_secret: config.calypso.oauth.clientSecret,
